...
class StudentStatsAssessment(models.Model):
    ...
    def calculate_final_assess_percentage(self):
        if not self.total_questions_taken:
            self.final_assess_percentage = 0
        else:
            percentage = (self.total_questions_score / self.total_questions_taken) * 100
            self.final_assess_percentage = round(percentage, 2)
        self.save(update_fields=["final_assess_percentage"])
        return self.final_assess_percentage
