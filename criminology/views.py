...
def homepage(request):
    ...
    if active_year:
        ...
        for record in assessment_records:
            # FIXED: Calculate total from actual categories if category_total is 0
            if record.category_total is None or record.category_total == 0:
                total_questions = (
                    record.remembering + record.creating + record.understanding +
                    record.applying + record.analyzing + record.evaluating
                )
            else:
    ...
    if active_year:
        ...
        if not subject_count_percentage_data.exists():
            print("⚠️ No SubjectCountPercentage data found. Generating from assessment data...")
            
            subjects_used = AnswerKeyAssessment.objects.filter(
                academic_year=active_year
            ).values_list('subject', flat=True).distinct()
            
            for subject_id in subjects_used:
                if subject_id:
                    subject = Subject.objects.get(id=subject_id)
                    
                    answer_keys = AnswerKeyAssessment.objects.filter(
                        academic_year=active_year,
                        subject=subject
                    )
                    ...
                    for answer_key in answer_keys:
                        stat = CorrectAndWrongCountsPerItem.objects.filter(
                            assessment_id=answer_key.assessment_exam_id,
                            number=answer_key.number
                        ).first()
                        
                        if stat:
                            total_correct += stat.correct_counts
                    
                    if total_questions > 0:
                        percentage = round((total_correct / total_questions), 2)
                        subject_data.append({
    ...
    print(f"📊 Subject Data Count: {len(subject_data)}")
    ...
    # 1. Question Performance Analysis - FIXED: Get from ALL CorrectAndWrongCountsPerItem
    question_performance = []
    
    print(f"🔍 Total CorrectAndWrongCountsPerItem records: {CorrectAndWrongCountsPerItem.objects.count()}")
    
    # Get ALL question stats (not just from assessment_records)
    all_question_stats = CorrectAndWrongCountsPerItem.objects.all()
    
    for stat in all_question_stats:
        total_attempts = stat.correct_counts + stat.wrong_counts
        if total_attempts > 0:
            difficulty = round((stat.correct_counts / total_attempts) * 100, 2)
            question_performance.append({
                'question': stat.question_description[:80] + '...' if len(stat.question_description) > 80 else stat.question_description,
                'number': stat.number,
    ...
    print(f"🔍 Total question performance entries: {len(question_performance)}")
    
    # Sort and get top 5 easiest and hardest
    question_performance.sort(key=lambda x: x['difficulty'])
    hardest_questions = question_performance[:5]
    easiest_questions = question_performance[-5:][::-1] if len(question_performance) >= 5 else []
    ...
    if active_year and enrolled_students_count > 0:
        for record in assessment_records:
            ...
            completion_rate = round((students_completed / enrolled_students_count) * 100, 2)
    ...
    print(f"📊 Assessment Completion entries: {len(assessment_completion)}")

    # 4. Pass/Fail Rate - Check ALL StudentStatsAssessment (not just active_year)
    pass_fail_data = []
    
    # Get all assessment IDs that have score data
    all_assessment_ids_with_scores = StudentStatsAssessment.objects.values_list(
        'exam_id', flat=True
    ).distinct()
    
    print(f"🔍 Assessment IDs with scores: {list(all_assessment_ids_with_scores)}")
    
    for record in assessment_records:
        # Try to get scores for this assessment (check both with and without active_year filter)
        scores = StudentStatsAssessment.objects.filter(exam_id=record.assessment_id)
        
        total_students = scores.count()
        
        print(f"📊 Assessment {record.assessment_id}: {total_students} students")
        
        if total_students > 0:
            passed = 0
            for s in scores:
                if s.total_questions_taken and s.total_questions_taken > 0:
                    percentage = (s.total_questions_score / s.total_questions_taken) * 100
                    if percentage >= 60:
                        passed += 1
            ...
            print(f"   Passed: {passed}, Failed: {failed}, Rate: {pass_rate}%")
    ...
    print(f"📊 Pass/Fail Data entries: {len(pass_fail_data)}")

    # 5. Questions by Status
    ...
    low_p_questions = []
    if active_year:
        # Get ALL assessment IDs (not just from active_year)
        all_assessment_ids = list(CorrectAndWrongCountsPerItem.objects.values_list('assessment_id', flat=True).distinct())
    ...
    print("="*50)
    print(f"✅ Bloom Chart Data: {len(bloom_data)} categories")
    print(f"✅ Assessment Summaries: {len(assessment_summaries)}")
    print(f"✅ Subject Data: {len(subject_data)} subjects")
    print(f"✅ Difficulty Distribution: {sum(difficulty_distribution.values())} questions")
    print(f"✅ Pass/Fail Data: {len(pass_fail_data)} assessments")
    print(f"✅ Completion Data: {len(assessment_completion)} assessments")
    print(f"✅ Question Bank: {len(subject_question_bank)} subjects")
    print(f"✅ Hardest Questions: {len(hardest_questions)}")
    print(f"✅ Easiest Questions: {len(easiest_questions)}")
    print("="*50 + "\n")
...
def print_generated_assessment(request, assessment_exam_id):
    assessment = AnswerKeyAssessment.objects.filter(
    ...
    selected_view = request.GET.get('view', 'questions')
    auto_print = request.GET.get('export') == 'pdf'

    assessment_items = list(assessment)
    chunked_items = [
        assessment_items[index:index + 10]
        for index in range(0, len(assessment_items), 10)
    ]
...
def print_generated_tableOfSpecification(request, tos_exam_id):
    tos = AnswerKeyTableOfSpecification.objects.filter(tos_exam_id=tos_exam_id).order_by('number')
    ...
    tos_items = list(tos)
    chunked_items = [
        tos_items[index:index + 10]
        for index in range(0, len(tos_items), 10)
    ]
...
def modules_create_topic_final(request):
    if request.method == 'POST':
        ...
        if not subject_topic_modules_id or not topic_name_modules:
            messages.error(request, 'All fields are required.')
            return redirect(reverse('modulessubtopic'))

        try:
            subject_topic_modules = Subject.objects.get(id=subject_topic_modules_id)
        except (Subject.DoesNotExist, ValueError, TypeError):
            messages.error(request, 'The specified subject does not exist.')
...
def modules_create_subtopic_final(request):
    if request.method == 'POST':
        ...
        try:
            topic_subtopic_modules = Topic.objects.get(id=topic_subtopic_modules_id)
...
def table_of_specification_update(request, group_id):
    ...
    if not tos_entries.exists():
        messages.error(request, "No Table of Specification entries found for the selected group.")
        return redirect(reverse('table_of_secification'))
    ...
    for entry in tos_entries:
        ...
        topics[topic.id]['pwd_total'] += entry.pwd
            ...
            with transaction.atomic():
                for entry in entries:
                    ...
                    if entry.row_id is not None:
                        PercentageWeightPerTos.objects.update_or_create(
                            group_id=group_id,
                            row_id=entry.row_id,
                            defaults={'pwd': entry.pwd},
                        )
        except ValidationError as exc:
            messages.error(request, exc.message)
            return redirect(reverse('table_of_specification_update', args=[group_id]))
...
def masterfilestudentscreate(request):
    ...
    if request.method == 'POST':
        ...
        posted_studentid = request.POST.get('studentId')
        studentid = posted_studentid or generate_unique_student_id()
        acadsyear = AcademicYear.objects.filter(pk=acadyear_id).first() if acadyear_id else None

        if Students.objects.filter(studentschoolid=schoolid).exists():
            messages.error(request, 'Student school ID already exists.')
...
def masterfilestudentsupdate(request, pk):
    ...
    if request.method == 'POST':
        ...
        studentschoolid = request.POST.get('studentschoolid_update')

        if (
            studentschoolid
            and Students.objects.filter(studentschoolid=studentschoolid)
            .exclude(id=pk)
            .exists()
        ):
            messages.error(request, 'Student school ID already exists.')
        ...
        student.studentschoolid = studentschoolid
...
def check_tos(request):
    ...
    base_queryset = AnswerKeyTableOfSpecification.objects.all()
    if active_academic_year:
        base_queryset = base_queryset.filter(academic_year=active_academic_year)

    representatives = base_queryset.values('tos_exam_id').distinct()

    representative_entries = []
    for tos_exam in representatives:
        if query:
            representative = base_queryset.filter(
...
def check_assessment(request):
    ...
    base_queryset = AnswerKeyAssessment.objects.all()
    if active_academic_year:
        base_queryset = base_queryset.filter(academic_year=active_academic_year)

    representatives = base_queryset.values('assessment_exam_id').distinct()
...
def import_csv_tos(request, tos_exam_id):
    ...
    if not active_year:
        return render(request, "assets/examchecker_tos.html", {
            'tos_exam_id': tos_exam_id,
            'exam_id': exam_id,
            'error': "No active academic year found. Please set an active academic year.",
    ...
    if request.method == "POST" and request.FILES.get("csv_file"):
        ...
        try:
            ...
            if data.empty:
                return render(request, "assets/examchecker_tos.html", {
                    "error": "No valid student rows found in the uploaded CSV.",
                    'exam_id': exam_id,
                    'selected_exam_tos_id': exam_tos_id,
                    'exam_ids': get_representative_exam_ids()
                })
            ...
            if not question_columns:
                return render(request, "assets/examchecker_tos.html", {
                    "error": "No question columns (e.g., Q1, Q2, ...) found in the uploaded CSV.",
                    'exam_id': exam_id,
                    'selected_exam_tos_id': exam_tos_id,
                    'exam_ids': get_representative_exam_ids()
                })
            ...
            data = data.dropna(subset=['External Id'])
            data['External Id'] = data['External Id'].astype(int)
            ...
            for student in Students.objects.all():
                students_dict[student.studentid] = {
                    'studentid': student.studentid,
                    'studentschoolid': student.studentschoolid,
                    'firstname': student.firstname,
                    'lastname': student.lastname
                }
                if student.studentschoolid is not None and student.studentschoolid not in students_dict:
                    students_dict[student.studentschoolid] = {
                        'studentid': student.studentid,
                        'studentschoolid': student.studentschoolid,
                        'firstname': student.firstname,
                        'lastname': student.lastname
            ...
            def get_student_info(row):
                return students_dict.get(row['External Id'])

            def get_first_name(row):
                student_info = get_student_info(row)
                if student_info:
                    return student_info['firstname']
                return row['First Name']

            def get_last_name(row):
                student_info = get_student_info(row)
                if student_info:
                    return student_info['lastname']
                return row['Last Name']

            def get_student_id(row):
                student_info = get_student_info(row)
                if student_info:
                    return student_info['studentid']
                return row['External Id']

            def get_student_school_id(row):
                student_info = get_student_info(row)
                if student_info:
                    return student_info['studentschoolid']
                return row['External Id']

            merged_data = data.copy()
            merged_data['Display First Name'] = merged_data.apply(get_first_name, axis=1)
            merged_data['Display Last Name'] = merged_data.apply(get_last_name, axis=1)
            merged_data['studentid'] = merged_data.apply(get_student_id, axis=1)
            merged_data['studentschoolid'] = merged_data.apply(get_student_school_id, axis=1)
            merged_data['First Name'] = merged_data['Display First Name']
            merged_data['Last Name'] = merged_data['Display Last Name']
            ...
            question_numbers = list(question_mapping.values())
            tos_entries = AnswerKeyTableOfSpecification.objects.filter(
                tos_exam_id=tos_exam_id,
                number__in=question_numbers
            ).select_related('topic', 'subtopic', 'question')
            tos_entries_by_number = {entry.number: entry for entry in tos_entries}
            ...
            for question, question_number in question_mapping.items():
                ...
                if correct_count >= passing_threshold:
                    answer_key_entry = tos_entries_by_number.get(question_number)
                    if not answer_key_entry:
                        continue
            ...
            html_table = sorted_data[['Rank', 'Display First Name', 'Display Last Name', 'Class', 'Total Score']].rename(
                columns={'Display First Name': 'First Name', 'Display Last Name': 'Last Name'}
            ).to_html(
            ...
            session_data = sorted_data.copy()
            session_data = session_data.applymap(lambda x: int(x) if isinstance(x, np.int64) else x)
            request.session['sorted_data'] = session_data.to_dict('records')
            ...
            for index, row in merged_data.iterrows():
                ...
                student_name = f"{row['Display First Name']} {row['Display Last Name']}"
                for col, number in question_mapping.items():
                    tos_entry = tos_entries_by_number.get(number)
                    if not tos_entry:
                        continue
            ...
            for student_name in merged_data.apply(lambda x: f"{x['Display First Name']} {x['Display Last Name']}", axis=1).unique():
                if student_name not in student_topic_subtopic_counts:
                    student_topic_subtopic_counts[student_name] = {}
...
def save_data_tos(request):
    if request.method == "POST":
        active_year = AcademicYear.objects.filter(status=1).first()
        if not active_year:
            messages.error(request, "No active academic year found. Please set an active academic year.")
        ...
        for row in sorted_data:
            student = Students.objects.filter(studentid=row.get('studentid')).first()
            if not student:
                student = Students.objects.filter(studentschoolid=row.get('studentschoolid')).first()
            if not student:
                continue
...
def import_csv_assessment(request, assessment_exam_id):
    ...
    if request.method == "POST" and request.FILES.get("csv_file"):
        ...
        try:
            ...
            numeric_columns = question_columns + ['Num Correct', 'Num Questions']
            ...
            def get_first_name(row):
                if row['External Id'] in students_dict:
                    return students_dict[row['External Id']]['firstname']
                return row['First Name']
            
            def get_last_name(row):
                if row['External Id'] in students_dict:
                    return students_dict[row['External Id']]['lastname']
                return row['Last Name']
            
            merged_data = data.copy()
            merged_data['Display First Name'] = merged_data.apply(get_first_name, axis=1)
            merged_data['Display Last Name'] = merged_data.apply(get_last_name, axis=1)
            merged_data['studentid'] = merged_data['External Id']
            ...
            top_5_students = sorted_data.head(5)[['Display First Name', 'Display Last Name', 'Total Score', 'Rank']].rename(
                columns={'Display First Name': 'First Name', 'Display Last Name': 'Last Name'}
            ).to_dict('records')
            ...
            print("Sample session data:", session_data[['Display First Name', 'Display Last Name', 'Total Score', 'Rank']].head())
    ...
    if request.method == "POST":
        ...
        with transaction.atomic():
            try:
                ...
                for row in sorted_data:
                    first_name = row.get('Display First Name') or row.get('First Name')
                    last_name = row.get('Display Last Name') or row.get('Last Name')
                    student_score = {
                        'academic_year': active_year,
                        'score': row['Total Score'],
                        'rank_position': row['Rank'],
                ...
                for row in sorted_data:
                    ...
                    if not student:
                        student = Students.objects.filter(studentschoolid=row.get('studentschoolid')).first()
                    if student:
                        assessment_count = StudentStatsAssessment.objects.filter(
                            student=student,
                            academic_year=active_year
                        ).count() + 1
...
def get_unique_assessments_students_score():
    ...
    base_queryset = StudentsScoreAssessment.objects.select_related(
        'subject__assessment',
        'subject__subject'
    )
    if active_academic_year:
        base_queryset = base_queryset.filter(academic_year=active_academic_year)
    base_queryset = base_queryset.order_by('-subject__created_at')

    assessments = []
    seen_exam_ids = set()
    for row in base_queryset:
        if row.exam_id not in seen_exam_ids:
...
def get_unique_table_of_specifications_students_score():
    ...
    answer_key_queryset = AnswerKeyTableOfSpecification.objects.select_related(
        'subject',
        'topic'
    )
    if active_academic_year:
        answer_key_queryset = answer_key_queryset.filter(
            academic_year=active_academic_year
        )
    answer_key_queryset = answer_key_queryset.order_by('-created_at')
    answer_key_representatives = {}
    for answer_key in answer_key_queryset:
        if answer_key.tos_exam_id not in answer_key_representatives:
            answer_key_representatives[answer_key.tos_exam_id] = answer_key

    table_of_specifications = StudentsScoreTos.objects.select_related(
        'subject',
        'subject__topic'
    )
    if active_academic_year:
        table_of_specifications = table_of_specifications.filter(academic_year=active_academic_year)
    table_of_specifications = table_of_specifications.order_by('-subject__created_at')
    unique_tos = {}
    for tos in table_of_specifications:
        if tos.exam_id not in unique_tos:
            answer_key_tos = answer_key_representatives.get(tos.exam_id) or tos.subject
            unique_tos[tos.exam_id] = {
                'exam_id': tos.exam_id,
                'subject_name': answer_key_tos.subject.subject_name if answer_key_tos else None,
                'topic_name': answer_key_tos.topic.topic_name if answer_key_tos and answer_key_tos.topic else None,
                'period': tos.period,
                'created_at': answer_key_tos.created_at if answer_key_tos else None,
            }
    return list(unique_tos.values())
...
def students_stats(request):
    ...
    stats_queryset = StudentStatsAssessment.objects.all()
    tos_queryset = StudentStatsTos.objects.all()
    if active_academic_year:
        stats_queryset = stats_queryset.filter(academic_year=active_academic_year)
        tos_queryset = tos_queryset.filter(student__academic_year=active_academic_year)

    max_assessment_count = stats_queryset.aggregate(
        Max('assessment_taken_count')
    )['assessment_taken_count__max']
    
    if max_assessment_count is not None:
        stud_stats = stats_queryset.filter(assessment_taken_count=max_assessment_count)
        stud_stats = sorted(stud_stats, key=lambda x: x.score_percentage, reverse=True)

    for stat in tos_queryset:
...
def students_all_stats_assessment(request, studentid):
    try:
        ...
        if active_academic_year:
            stud_stats = stud_stats.filter(academic_year=active_academic_year)

        exam_ids = list(stud_stats.values_list('exam_id', flat=True))
        assessments = Assessment.objects.select_related('topic', 'subject').filter(
            assessment_id__in=exam_ids
        )
        if active_academic_year:
            assessments = assessments.filter(academic_year=active_academic_year)

        assessments_map = {assessment.assessment_id: assessment for assessment in assessments}
        exam_to_percentage = {
            pwa.assessment_id: float(pwa.percentage_w_assess)
            for pwa in PercentageWeightPerAssessment.objects.filter(
                assessment_id__in=exam_ids,
                status=0
            )
            if pwa.percentage_w_assess is not None
        }

        full_name = f"{student.firstname} {student.lastname}"

        total_questions_taken = stud_stats.aggregate(total=Sum('total_questions_taken'))['total'] or 0
        assessment_counts = int(total_questions_taken / 100) if total_questions_taken else 0

        assessment_stats = []
        subject_totals = {}
        for stat in stud_stats:
            assessment = assessments_map.get(stat.exam_id)
            subject_code = "N/A"
            subject_name = "N/A"
            topic_name = "N/A"
            if assessment:
                ...
                if assessment.topic:
                    topic_name = assessment.topic.topic_name
            ...
            percentage_value = exam_to_percentage.get(stat.exam_id, 0.0)
            subject_totals[subject_code]['total_percentage_w_assess'] += percentage_value

            score_percentage = (
                round((stat.total_questions_score / stat.total_questions_taken) * 100, 2)
                if stat.total_questions_taken
                else 0.0
            )
            assessment_stats.append({
                ...
                'percentage_w_assess': exam_to_percentage.get(stat.exam_id),
                'total_questions_score': stat.total_questions_score,
                'total_questions_taken': stat.total_questions_taken,
                'contribution': stat.contribution,
                'score_percentage': score_percentage,
            })

        assessment_stats.sort(
            key=lambda stat: (stat['subject_code'] or "", stat['exam_id'] or 0)
        )
        ...
        for subject_code, totals in subject_totals.items():
            ...
            if totals['total_percentage_w_assess'] == 0:
                totals['status'] = "No Data"
                totals['status_class'] = "text-gray-500"
            elif totals['passing_rate'] >= 60:
                totals['status'] = "Passed"
                totals['status_class'] = "text-green-600"
            else:
                totals['status'] = "Failed"
                totals['status_class'] = "text-red-600"
        ...
        subjects = {assessment.subject.subject_code for assessment in assessments if assessment.subject}
